...
#define CODING_LEN 10

char ret[2][10] = {"UTF-8","GBK"}; 

char *detect_encoding_format(const char* str)
{
...
int encoding()
{
    ...
    iconv_t cd = iconv_open("UCS-2LE", det_ret);  // UCS-2LE表示UCS-2 Little Endian
    if (cd == (iconv_t)-1) {
        perror("iconv_open");
        exit(1);
    }
